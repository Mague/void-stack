...
/// Look up `key` in the lazily built map for `entries`. Unknown keys fall
/// back to the key itself.
fn lookup<'a>(
    cell: &'static OnceLock<TranslationMap>,
    entries: &'static [(&'static str, &'static str)],
    key: &'a str,
) -> &'a str {
    let map = cell.get_or_init(|| entries.iter().copied().collect());
    map.get(key).copied().unwrap_or(key)
}

fn es(key: &str) -> &str {
    static MAP: OnceLock<TranslationMap> = OnceLock::new();
    lookup(&MAP, ES_ENTRIES, key)
}

fn en(key: &str) -> &str {
    static MAP: OnceLock<TranslationMap> = OnceLock::new();
    lookup(&MAP, EN_ENTRIES, key)
}

static ES_ENTRIES: &[(&str, &str)] = &[
    // Header
    ("projects", "proyectos"),
    ("services", "servicios"),
    ("session", "sesion"),
    ...
    (
        "analysis.run_hint",
        "Presiona R para analizar el proyecto actual",
    ),
    ("analysis.running", "Analizando..."),
    ("analysis.complete", "Analisis completado"),
    (
        "analysis.no_code",
        "No se encontraron archivos fuente analizables",
    ),
    // Security
    ...
    (
        ...
        "Sin problemas de seguridad encontrados!",
    ),
    (
        "security.run_hint",
        "Presiona R para ejecutar auditoria de seguridad",
    ),
    ...
    (
        "debt.run_hint",
        "Presiona R para escanear marcadores TODO/FIXME/HACK",
    ),
    ...
    (
        "suggest.running",
        "Generando sugerencias (requiere Ollama)...",
    ),
    ("suggest.generated", "sugerencias generadas"),
    (
        "suggest.no_ollama",
        "Ollama no disponible — mostrando contexto",
    ),
    // Log filter
    ...
    (
        "stats.run_hint",
        "Presiona R para cargar estadisticas de ahorro",
    ),
    ("stats.ops", "ops"),
...
static EN_ENTRIES: &[(&str, &str)] = &[
    ...
    (
        "suggest.running",
        "Generating suggestions (requires Ollama)...",
    ),
    ("suggest.generated", "suggestions generated"),
    (
        "suggest.no_ollama",
        "Ollama not available — showing analysis context",
    ),
    // Log filter
