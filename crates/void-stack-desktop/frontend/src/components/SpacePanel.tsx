...
const categoryIcon = (cat: string) => {
  switch (cat) {
    case 'Dependencias': case 'Dependencies': return <Package size={12} />
    case 'Build': return <FolderArchive size={12} />
    case 'Caché global': case 'Global cache': return <Database size={12} />
    case 'Modelos AI': case 'AI Models': return <Cpu size={12} />
...
const categoryColor = (cat: string) => {
  switch (cat) {
    case 'Dependencias': case 'Dependencies': return 'var(--cyan)'
    case 'Build': return 'var(--amber)'
    case 'Caché global': case 'Global cache': return 'var(--purple)'
    case 'Modelos AI': case 'AI Models': return 'var(--green)'
...
export default function SpacePanel({ project, projectEntries, setProjectEntries, globalEntries, setGlobalEntries }: Props) {
  ...
  const handleDelete = async (entry: SpaceEntry) => {
    if (!confirm(t('space.confirmDelete', { name: entry.name, size: entry.size_human, hint: entry.restore_hint }))) return
    ...
    <div className="panel">
      <div className="panel-header">
        ...
        <button className="btn btn-primary" onClick={scanAll} disabled={scanning}>
          {scanning ? <><span className="loading-spinner" /> {t('space.scanning')}</> : <><RefreshCw size={12} /> {t('space.scan')}</>}
        </button>
      </div>
        ...
        <>
          ...
          <div className="space-summary">
            <div className="space-total">
              ...
              <span className="space-total-label">{t('space.totalRecoverable')}</span>
            </div>
            <div className="space-breakdown">
              {totalProject > 0 && (
                <span className="space-breakdown-item">
                  <span style={{ color: 'var(--cyan)' }}>{t('space.project')}:</span> {formatSize(totalProject)}
                </span>
              )}
              {totalGlobal > 0 && (
                <span className="space-breakdown-item">
          ...
          {projectEntries.length > 0 && (
            <div className="space-section">
              <h3 className="space-section-title">{t('space.project')}</h3>
          ...
          {globalEntries.length > 0 && (
            <div className="space-section">
              <h3 className="space-section-title">{t('space.globalCache')}</h3>
              <div className="space-entries">
