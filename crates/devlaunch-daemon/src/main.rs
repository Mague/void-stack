...
use devlaunch_proto::pb;
use devlaunch_proto::DevLaunchServer;

use crate::lifecycle::{DaemonInfo, is_process_alive, read_pid_file, remove_pid_file, write_pid_file};
use crate::server::DevLaunchService;

const DEFAULT_PORT: u16 = 50051;

#[derive(Parser)]
#[command(name = "devlaunch-daemon", about = "DevLaunch background daemon")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "devlaunch=info".parse().unwrap()),
        )
        .init();
...
async fn cmd_start(path: &str, port: u16) -> Result<()> {
    ...
    let service = DevLaunchService::new(manager.clone(), log_tx);
    ...
    Server::builder()
        .add_service(DevLaunchServer::new(service))
...
async fn cmd_stop() -> Result<()> {
    ...
    match devlaunch_proto::DevLaunchClient::connect(addr).await {
        Ok(mut client) => {
        ...
        Some(info) => {
            ...
            match devlaunch_proto::DevLaunchClient::connect(addr).await {
                Ok(mut client) => {
                    ...
                    println!("DevLaunch Daemon v{}", resp.version);
