...
pub struct DevLaunchService {
    manager: Arc<ProcessManager>,
    started_at: Instant,
    log_tx: broadcast::Sender<pb::LogEntry>,
}

impl DevLaunchService {
...
#[tonic::async_trait]
impl pb::dev_launch_server::DevLaunch for DevLaunchService {
    async fn start_all(
        &self,
    ...
    async fn start_one(
        ...
        let state = ServiceBackend::start_one(self.manager.as_ref(), name)
            .await
            .map_err(|e| match &e {
                devlaunch_core::error::DevLaunchError::ServiceNotFound { .. } => {
    ...
    async fn ping(
        ...
        let running = states
            .iter()
            .filter(|s| s.status == devlaunch_core::model::ServiceStatus::Running)
            .count() as u32;
