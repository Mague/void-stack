...
mod tests {
    ...
    fn test_install_git_hook_includes_graph_build() {
        ...
        install_git_hook(&project).unwrap();
        let hook = std::fs::read_to_string(dir.path().join(".git/hooks/post-commit")).unwrap();
        assert!(
            hook.contains("void index hooked --git-base HEAD~1"),
            "{hook}"
        );
        assert!(hook.contains("void graph-build hooked"), "{hook}");
    }
    ...
    fn test_install_git_hook_upgrades_old_index_only_hook() {
        ...
        std::fs::write(
            &hook_path,
            "#!/bin/sh\n# Auto-generated by void-stack\nvoid index hooked --git-base HEAD~1 2>/dev/null &\n",
        )
        .unwrap();
        let project = crate::model::Project {
            name: "hooked".into(),
        ...
        assert!(
            hook.contains("void graph-build hooked"),
            "old hook must be upgraded: {hook}"
        );
        // The old index-only line is replaced, not duplicated.
        assert_eq!(hook.matches("void index hooked").count(), 1, "{hook}");
    }
