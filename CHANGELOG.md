...
- **Structural graph analysis** (`--features structural`, also included in `full`) — Tree-sitter powered function-level call graph persisted at `.void-stack/structural.db`. Supports Rust, Python, JavaScript, TypeScript (+ TSX) and Go via their respective tree-sitter grammars. New public API in `void_stack_core::structural` (`parse_file`, `build_structural_graph`, `get_impact_radius`, `get_callers`, `get_callees`, `get_tests_for`, `search_nodes`) and three MCP tools: `build_structural_graph`, `get_impact_radius`, `query_graph`. Impact radius runs a bidirectional BFS via a recursive SQLite CTE so you can ask *"if I change file X, what else is affected?"*. Incremental re-builds skip files whose SHA-256 matches the cached one. 16 new tests
