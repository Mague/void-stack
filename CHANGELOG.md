...
### Added (test coverage)
- **~470 new unit tests** across void-stack-core (audit vuln_patterns/secrets/config_check/deps, vector_index chunker/voidignore/stats/db/search/indexer, docker generators, python detector, global-config scanner, drawio + api-routes + architecture + externals diagram scanners, best_practices mapping, complexity analyzer, import graph, process manager) — the workspace suite grew from 1249 to 1704 tests. void-stack-core line coverage measured at **78.3%** with `cargo llvm-cov` (workspace 65.5% — the TUI/CLI/MCP glue crates have no harness yet); `lcov.info` regenerated so the Analysis panel reflects the real number.
