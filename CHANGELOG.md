...
### Added (test coverage)
- **~600 new unit tests** — the workspace suite grew from 1249 to 1853. void-stack-core (audit, vector_index, docker generators, detectors, scanner, diagram scanners, best_practices, complexity, import graph, process manager) at **78.4%** line coverage; the glue crates finally got harnesses: TUI **0% → 60.9%** (ratatui `TestBackend` renders + a mock `ServiceBackend` for key handlers and app state), MCP **8.9% → 38.6%** (tool formatters over tempdir git repos, orchestration helpers, serde request-type roundtrips), CLI **7.6% → 33.0%** (clap parse self-checks + command handlers over registered tempdir fixtures). Workspace total **63.5% → 69.8%** (`cargo llvm-cov`); `lcov.info` regenerated so the Analysis panel reflects the real number.
