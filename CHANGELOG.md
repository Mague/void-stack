...
### Added (test coverage)
- **~710 new unit tests** — the workspace suite grew from 1249 to 1960. void-stack-core (audit, vector_index, docker generators, detectors, scanner, diagram scanners, best_practices, complexity, import graph, process manager, WSL runner branches, commit subjects, contract extractors) reached **79.0%** line coverage; the glue crates got real harnesses: TUI **0% → 83.0%** (ratatui `TestBackend` renders incl. populated data tabs + a mock `ServiceBackend` for key handlers and app state), MCP **8.9% → 54.0%** (tool formatters over tempdir git repos, debt/space/doctor/graph/projects tools over isolated config, serde request-type roundtrips), CLI **7.6% → 48.6%** (clap parse self-checks + command handlers incl. project lifecycle and `void commit` over registered tempdir fixtures), daemon **0% → 29.2%** (PID-file lifecycle). Workspace total **63.5% → 73.0%** (`cargo llvm-cov`); `lcov.info` regenerated so the Analysis panel reflects the real number.
...
### Fixed
- **God Class detector: three false-positive fixes** — `detect_god_classes` in `analyzer::patterns::antipatterns` now (1) excludes `#[cfg(test)] mod … { … }` blocks from LOC and function counts via brace-depth tracking in `rust_lang.rs`; (2) skips files flagged `is_hub` (re-export facade when >50% of production lines are `pub use` / `pub mod` / `mod X;`); (3) drops the `function_count` trigger on files containing `#[tool_router]` or `#[tool_handler]` — `has_framework_macros` flag, since one function per tool is inherent to the rmcp pattern. LOC check is kept in that case. New `is_hub` / `has_framework_macros` fields on `ModuleNode` and `ParseResult`, populated by the Rust parser; other languages default to `false`. 10 new tests. Eliminates ~17 false-positive God Class flags on the void-stack codebase (server.rs, vector_index/mod.rs, detector/mod.rs, log_filter.rs, best_practices/mod.rs, complexity.rs, cross_project.rs, docs/markdown.rs, history.rs, imports/mod.rs, audit/*.rs, claudeignore.rs, diagram/architecture/externals.rs, docker/*.rs, global_config.rs); remaining 4 flags are real.

### Refactored
- **`structural/parser.rs` 698 → 435 LOC, 34 → 17 funciones productivas** — Extracted the per-language AST dispatch into a new `structural/langs/` submodule with a `LanguageWalker` trait and five impls (`RustWalker`, `PythonWalker`, `JsWalker` for js/ts/tsx, `GoWalker`, `OthersWalker` for dart/java/php/c/cpp). `Walker::is_{class,function,call,import}_node` now delegates to `self.lang_walker.*`. Moved `language_for` and `load_language` into `langs/mod.rs` where they belong; moved `NodeKind`, `EdgeKind`, `StructuralNode`, `StructuralEdge`, `ParseResult` into a new `structural/model.rs`. Re-exports keep the public API identical. Fixes the God Class flag on `parser.rs`. 5 new tests on walker dispatch.

### Fixed
- **`watch_project` no longer passes `Some("HEAD")` to the indexer** — Filesystem-watch kicks the indexer on every save, not on commits. `Some("HEAD")` asks the indexer to diff HEAD against the working tree — which is empty whenever everything is already committed, so watch re-indexed nothing in practice. Switched to `None` so the indexer uses its SHA-256 content check instead. Regression-guarded by a source-literal test that refuses to let the string reappear.
- **`CODE_EXTENSIONS` trimmed** — Removed `json`, `yaml`, `yml`, `toml`, `txt`, `sh`, `bash`, `zsh`, `fish`, `ps1`, `bat`, `cmd`, `rst`, `adoc`, `makefile`, `justfile`. These are config or shell-boilerplate and drown real code in search results (package-lock files, schemas, helm values, etc.). Kept source languages plus `md` (READMEs), `proto`, `sql`, and `dockerfile`. Re-index of void-stack dropped from **263 files / 2619 chunks** to **239 / 2305** — ~13% fewer chunks, zero real-code loss. 2 new tests.
- **`cleanup_stale_chunks` also drops orphan `chunk_order` rows** — Deleting a file's chunks left its `hnsw_id` → `chunk_id` mapping intact. The HNSW layer would then return a chunk_id that no longer existed, producing phantom `semantic_search` results. Added `DELETE FROM chunk_order WHERE chunk_id NOT IN (SELECT id FROM chunks)` inside the same cleanup transaction. 1 new test seeds a chunk + order row, removes the file, asserts zero orphans.
- **Post-commit hook: `--git-base HEAD` → `--git-base HEAD~1`** — `install_git_hook` was writing `void index <project> --git-base HEAD` into `.git/hooks/post-commit`. Post-commit, `git diff HEAD` compares the new commit's tree against the working tree — which is empty, so nothing ever got re-indexed. Changed to `HEAD~1` so the hook diffs the new commit against its parent and picks up every file the commit touched. Test updated to require `HEAD~1` and reject bare `HEAD`.
- **Incremental re-index now drops newly-ignored files** — When `.voidignore` grew a pattern after an initial index, chunks for the now-excluded files stayed in the DB forever and kept showing up in `semantic_search` results. New `cleanup_stale_chunks` helper runs at the start of every incremental `index_project`, diffs `collect_indexable_files` against the set of files present in the DB, and `DELETE`s the chunks of anything no longer indexable (also handles deleted files and extensions removed from the allowlist). Force re-index already wiped everything, so it's a no-op there. 2 new tests: stale file is removed, no-op when all files are still current.
- **`is_test_cfg`: handle non-trivial `#[cfg(...)]` attributes** — The first `#[cfg(test)]` exclusion only matched the literal string; void-stack's own code uses `#[cfg(all(test, feature = "structural"))]` which slipped through, re-inflating `parser.rs` function_count. New helpers `split_cfg_attr` (paren-depth matching to find the attribute boundary) and `is_test_cfg` (word-boundary match for the `test` predicate) now catch `#[cfg(test)]`, `#[cfg(all(test, …))]`, `#[cfg(any(test, …))]`, while rejecting string-literal `"tests"` / `"testing"` false positives. 2 new tests.
...
- **Vector index: dependent file propagation** — When a file's content changes, `index_project` now also re-embeds files that import it (one-hop) via the existing `analyzer::imports::build_graph` dependency graph. Keeps importers' embeddings in sync with the definitions they reference. Exposed as `void_stack_core::vector_index::find_dependents`. External imports are ignored; non-Rust/Python/Go/Dart/JS projects degrade gracefully to no-op. 3 new tests
- **Vector index: chunk enrichment with import context** — Before embedding, each new chunk gets a 2-line structural summary prepended (`// Used by: …`, `// Imports: …`, capped at 3 names each) so embeddings capture dependency semantics. The enriched text is used only for the embedding; the text stored in SQLite — and shown in search results — stays the original code. Cached chunks keep their stored text to avoid invalidating prior embeddings. 4 new tests
- **Vector index: watch mode + git post-commit hook** — `watch_project` (MCP tool) uses `notify` v6 to follow file-system events and fires an incremental re-index with 500 ms debounce, so bursts from autoformat or save-all only trigger one rebuild. `install_index_hook` (MCP tool) writes a `.git/hooks/post-commit` that calls `void index <name> --git-base HEAD` after every commit; appending is idempotent. `unwatch_project` stops the watcher. Adds `notify = "6"` (gated by the `vector` feature). 4 new tests
- **Structural graph analysis** (`--features structural`, also included in `full`) — Tree-sitter powered function-level call graph persisted at `.void-stack/structural.db`. Supports **10 languages**: Rust, Python, JavaScript, TypeScript (+ TSX), Go, Dart, Java, PHP, C and C++ via their respective tree-sitter grammars. New public API in `void_stack_core::structural` (`parse_file`, `build_structural_graph`, `get_impact_radius`, `get_callers`, `get_callees`, `get_tests_for`, `search_nodes`) and three MCP tools: `build_structural_graph`, `get_impact_radius`, `query_graph`. Impact radius runs a bidirectional BFS via a recursive SQLite CTE so you can ask *"if I change file X, what else is affected?"*. Incremental re-builds skip files whose SHA-256 matches the cached one. Bumped tree-sitter to 0.25 for Dart grammar compatibility. 21 tests (5 new for Dart/Java/PHP/C/C++). Inspired by [code-review-graph](https://github.com/tirth8205/code-review-graph) (MIT) — AST node mappings and BFS query logic adapted to native Rust.
