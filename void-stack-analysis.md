...
| **Lenguaje** | Rust |
| **Modulos** | 207 archivos |
| **LOC** | 32547 lineas |
| **Deps externas** | 37 paquetes |

## Distribucion por Capas

| Capa | Archivos | LOC | % |
|------|----------|-----|---|
| Controller | 52 | 7795 | 23% |
| Service | 28 | 2712 | 8% |
| Repository | 8 | 1485 | 4% |
| Model | 19 | 3291 | 10% |
| Utility | 88 | 14206 | 43% |
...
- **Fat Controller**: Controller 'crates/void-stack-mcp/src/server.rs' tiene 489 LOC — demasiada logica
  - *Sugerencia*: Mover la logica de negocio a una capa de servicio
- **Fat Controller**: Controller 'crates/void-stack-core/src/analyzer/imports/classifier/signals.rs' tiene 1009 LOC — demasiada logica
  - *Sugerencia*: Mover la logica de negocio a una capa de servicio
- **Excessive Coupling**: 'crates/void-stack-core/src/lib.rs' importa 24 modulos (fan-out alto)
...
- **God Class**: 'crates/void-stack-core/src/structural/parser.rs' es demasiado grande (698 LOC y 34 funciones)
  - *Sugerencia*: Dividir 'crates/void-stack-core/src/structural/parser.rs' en modulos mas pequenos con responsabilidades claras
...
- **Fat Controller**: Controller 'crates/void-stack-mcp/src/tools/projects.rs' tiene 256 LOC — demasiada logica
  - *Sugerencia*: Mover la logica de negocio a una capa de servicio
- **Fat Controller**: Controller 'crates/void-stack-mcp/src/tools/search.rs' tiene 335 LOC — demasiada logica
  - *Sugerencia*: Mover la logica de negocio a una capa de servicio
...
- **Excessive Coupling**: 'crates/void-stack-core/src/runner/docker.rs' importa 13 modulos (fan-out alto)
  - *Sugerencia*: Reducir dependencias usando inyeccion de dependencias o fachadas
- **Excessive Coupling**: 'crates/void-stack-mcp/src/tools/mod.rs' importa 12 modulos (fan-out alto)
  - *Sugerencia*: Reducir dependencias usando inyeccion de dependencias o fachadas
...
- **Excessive Coupling**: 'crates/void-stack-core/src/runner/local.rs' importa 11 modulos (fan-out alto)
  - *Sugerencia*: Reducir dependencias usando inyeccion de dependencias o fachadas
- **Excessive Coupling**: 'crates/void-stack-mcp/src/tools/search.rs' importa 11 modulos (fan-out alto)
  - *Sugerencia*: Reducir dependencias usando inyeccion de dependencias o fachadas
...
| `crates/void-stack-core/src/structural/parser.rs` | Utility | 698 | 6 | 34 |
...
| `crates/void-stack-core/src/vector_index/indexer.rs` | Utility | 584 | 1 | 15 |
| `crates/void-stack-core/src/analyzer/complexity.rs` | Utility | 490 | 2 | 16 |
| `crates/void-stack-mcp/src/server.rs` | Controller | 489 | 1 | 45 |
...
| `crates/void-stack-core/src/audit/config_check.rs` | Config | 355 | 0 | 8 |
| `crates/void-stack-core/src/space/mod.rs` | Model | 353 | 2 | 9 |
| `crates/void-stack-core/src/analyzer/best_practices/mod.rs` | Utility | 342 | 7 | 18 |
| `crates/void-stack-mcp/src/tools/search.rs` | Controller | 335 | 0 | 18 |
| `crates/void-stack-tui/src/i18n.rs` | Model | 335 | 1 | 5 |
...
| `crates/void-stack-core/src/docker/generate_compose.rs` | Repository | 294 | 1 | 8 |

*... y 177 módulos más (ordenados por LOC, mostrando top 30)*
...
**Promedio**: 3.5 | **Funciones analizadas**: 1969 | **Funciones complejas (>=10)**: 189
...
- **Fat Controller**: Controller 'src/server.rs' tiene 489 LOC — demasiada logica
  - *Sugerencia*: Mover la logica de negocio a una capa de servicio
...
- **Fat Controller**: Controller 'src/tools/projects.rs' tiene 256 LOC — demasiada logica
  - *Sugerencia*: Mover la logica de negocio a una capa de servicio
- **Fat Controller**: Controller 'src/tools/search.rs' tiene 335 LOC — demasiada logica
  - *Sugerencia*: Mover la logica de negocio a una capa de servicio
- **No Service Layer**: Proyecto tiene 14 controllers pero ninguna capa de servicio
  - *Sugerencia*: Crear una capa de servicios para separar la logica de negocio de los endpoints
- **Excessive Coupling**: 'src/tools/mod.rs' importa 12 modulos (fan-out alto)
  - *Sugerencia*: Reducir dependencias usando inyeccion de dependencias o fachadas
- **Excessive Coupling**: 'src/tools/search.rs' importa 11 modulos (fan-out alto)
  - *Sugerencia*: Reducir dependencias usando inyeccion de dependencias o fachadas
...
| `src/tools/mod.rs` | Controller | 47 | 0 | 3 |
...
**Promedio**: 2.9 | **Funciones analizadas**: 102 | **Funciones complejas (>=10)**: 7
...
| `list_doc_files`  | `mod.rs` | 31 | 7 | 19 |
| `query_graph`  | `graph.rs` | 97 | 6 | 26 |
| `add_project`  | `projects.rs` | 57 | 6 | 75 |
| `index_project_codebase`  | `search.rs` | 19 | 6 | 43 |
| `semantic_search`  | `search.rs` | 81 | 6 | 51 |
| `read_project_docs`  | `docs.rs` | 11 | 5 | 57 |
| `save_debt_snapshot`  | `debt.rs` | 8 | 5 | 40 |
| `scan_directory`  | `projects.rs` | 177 | 5 | 43 |
...
| Archivo | Capa | LOC | Clases | Funciones |
|---------|------|-----|--------|----------|
| `src/server.rs` | Utility | 176 | 1 | 12 |
...
| **Patron** | Layered (confianza: 80%) |
| **Lenguaje** | Rust |
| **Modulos** | 138 archivos |
| **LOC** | 22196 lineas |
| **Deps externas** | 24 paquetes |

## Distribucion por Capas

| Capa | Archivos | LOC | % |
|------|----------|-----|---|
| Controller | 10 | 1797 | 8% |
| Service | 24 | 2660 | 11% |
| Repository | 9 | 2069 | 9% |
| Model | 11 | 1670 | 7% |
| Utility | 71 | 10856 | 48% |
...
- **Fat Controller**: Controller 'src/analyzer/imports/classifier/signals.rs' tiene 1009 LOC — demasiada logica
  - *Sugerencia*: Mover la logica de negocio a una capa de servicio
- **Excessive Coupling**: 'src/lib.rs' importa 24 modulos (fan-out alto)
  - *Sugerencia*: Reducir dependencias usando inyeccion de dependencias o fachadas
...
- **God Class**: 'src/structural/parser.rs' es demasiado grande (698 LOC y 34 funciones)
  - *Sugerencia*: Dividir 'src/structural/parser.rs' en modulos mas pequenos con responsabilidades claras
...
| `src/structural/parser.rs` | Utility | 698 | 6 | 34 |
...
| `src/audit/config_check.rs` | Config | 355 | 0 | 8 |
...
| `src/analyzer/cross_project.rs` | Utility | 249 | 2 | 10 |
| `src/vector_index/chunker.rs` | Utility | 243 | 0 | 6 |
| `src/vector_index/db.rs` | Repository | 238 | 0 | 10 |
| `src/docker/terraform.rs` | Service | 235 | 0 | 8 |

*... y 108 módulos más (ordenados por LOC, mostrando top 30)*
...
**Promedio**: 3.2 | **Funciones analizadas**: 1660 | **Funciones complejas (>=10)**: 150
